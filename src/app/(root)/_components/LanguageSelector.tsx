  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="group relative flex items-center gap-3 px-3 py-2 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700 w-full sm:w-48 min-h-[44px]"
      >
        <div
        ...
        <span className="text-gray-200 text-sm sm:min-w-[80px] text-left group-hover:text-white transition-colors">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-full sm:w-64 bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50"
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked = false; // All languages unlocked
                return (
                  <motion.div
                    key={lang.id}
                    initial={{ opacity: 0 }}
